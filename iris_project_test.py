...
class TestIrisPipeline(unittest.TestCase):
    ...
    def testDataLoading(self):
        ...
        self.assertEqual(len(self.target_names), 3) # target names are the three species

    # bad practice - don't test 2 things in one test (if breaks don't know what broke the code)
    def testMissingAndDuplicates(self):
    ...
    def testScaling(self):
        ...
        self.assertAlmostEqual(X_train_scaled.mean(), 0, delta=0.1) # This data should be about 0
        self.assertAlmostEqual(X_train_scaled.std(), 1, delta=0.1) # This data should be about 1

    def testModelTrainingAndEvaluation(self):
        X_train, X_test, y_train, y_test = train_test_split(self.X, self.y, test_size=0.2, random_state=42)
        scaler = StandardScaler()
        X_train_scaled = scaler.fit_transform(X_train)
        X_test_scaled = scaler.transform(X_test)
        ...
        # Test confusion matrix - typically don't bother with this
